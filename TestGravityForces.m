%This script was created to test the gravity forces acting on the
%Airplane.  You will need to create the functions 
%
% [fxg,fyg,fzg] = gravityForces(m,e0,e1,e2,e3)
%    and
% DrawAirplane(xNorth, yUp, zEast, e0,e1,e2,e3)
%
%If done correctly, you should see your airplane with
% an arrow of length 9.81 pointing straight down
%
%Feel free to change the axis of rotation (eh) and the angle
% of rotation (varrho)
%
%The "mArrow3" function was created by Georg Stillfried

clear all
close all
clc

varrho = 90*pi/180; %rotation angle
eh = [-1,-3,2]; %axis of rotation
ehat = eh/norm(eh);
e0 = cos(varrho/2);
e1 = sin(varrho/2)*ehat(1);
e2 = sin(varrho/2)*ehat(2);
e3 = sin(varrho/2)*ehat(3);
m = 1; %(kg) mass
[fxg,fyg,fzg] = gravityForces(m,e0,e1,e2,e3)
...
DrawAirplane(0, 0, 0, e0,e1,e2,e3)
hold on 
mArrow3([0;0;0],R_ENU*fxI,'color','red')
title('The Arrow Should Point Straight Down')
